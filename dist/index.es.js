...
var momentFromValueOf = (obj, op = defaultOptions) => {
  ...
  let options = Object.assign({}, defaultOptions, op),
      ...
      o = options.newObject ? clone_1(obj) : obj;
  return recurse(o, propMatcher, valueMatcher, converter);
};

var recurse = (obj, propMatcher, valueMatcher, converter) => {
  if (typeof obj === "object") {
    lodash.keys(obj).map(k => {
      if (!obj.hasOwnProperty(k)) return;
      if (typeof obj[k] === 'object' && !moment.isMoment(obj[k])) recurse(obj[k], propMatcher, valueMatcher, converter);else if (propMatcher && propMatcher(k)) converter(obj, k);
    });
  }
  return obj;
};
...
var setupConverter = options => {
  return (obj, k) => {
    ...
    let m = i ? moment(i) : v;
    if (options.toFormat) m = m.format(options.toFormat);
...
var handleNewPropName = (obj, k, m, options) => {
  ...
  if (!options.newProperty) obj[k] = m;else {
    if (options.newProperty.name) obj[options.newProperty.name] = m;else {
